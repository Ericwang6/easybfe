...
#     parser.add_argument('--verbose', dest='verbose', help='Print as much information as possible. Useful for developers', action='store_true')
#     parser.add_argument('--version', action='version', version=f'EasyBFE - version {__version__}')
#     parser.add_argument('-d', '--directory', dest='directory', help='Path to the EasyBFE project', default='.')
...
#     add_ligands_parser.add_argument('-i', '--input', dest='input', required=True, nargs="+", help='Input sdf file')
#     add_ligands_parser.add_argument('-n', '--name', dest='name', help='Name of the ligand. Only valid when the input sdf contains one molecule, otherwise will use the name in .sdf file')
#     add_ligands_parser.add_argument('-p', '--protein_name', dest='protein_name', required=True, help='Name of the protein that the added ligand(s) belong to')
#     add_ligands_parser.add_argument('-f', '--forcefield', dest='forcefield', default='bcc',
#         help=(
#             'Ligand forcefield. Valid options: "gaff", "gaff2" (acpype supported) and "openff-*" series (openff supported). When adding only one ligand, ' 
#             'users can also specify a .prmtop/.top file as a customized forcefield for the ligand. Default is "gaff2"'
#         )
#     )
#     add_ligands_parser.add_argument('-c', '--charge', dest='charge_method', help='Charge method. Valid options: "bcc" (default, am1-bcc model), "gas" (gasteiger model, inaccurate, never use it in production)')
#     add_ligands_parser.add_argument('-m', '--num_workers', dest='num_workers', type=int, default=1, help='Number of process to run parametrization in parallel. Default is 1.')
#     add_ligands_parser.add_argument('--no-parametrize', dest='parametrize', action='store_false', help='Disable parametrization for the ligands. Only useful in tests or you want to build your own force fields afterwards')
#     add_ligands_parser.add_argument('--overwrite', dest='overwrite', action='store_true', help='Toggle to overwrite the existing ligand.')
#     add_ligands_parser.add_argument('--expt', dest='expt', 
...
#     add_protein_parser.add_argument('-i', '--input', dest='input', required=True, help='Path to the protein PDB file')
#     add_protein_parser.add_argument('-n', '--name', dest='name', required=True, help='Name of this protein model')
#     add_protein_parser.add_argument('--no-check-ff', dest='check_ff', action='store_false', help='Toggle to disable force field check.')
#     add_protein_parser.add_argument('--overwrite', dest='overwrite', action='store_true', help='Toggle to overwrite the exisiting protein.')

#     # Add perturbation
#     add_pert_parser = subparsers.add_parser('add_perturbation', help='Add perturbation')
#     add_pert_parser.add_argument('-p', '--protein_name', dest='protein_name', required=True)
#     add_pert_parser.add_argument('-l', '--list', dest='list', default=None, help='File contains list of perturabtions. In the file, each line contains two ligand names seperated by a whitespace')
#     add_pert_parser.add_argument('-c', '--config', dest='config', help='Configuration file', required=True)
#     add_pert_parser.add_argument('-m', '--num_workers', dest='num_workers', type=int, default=1, help='Number of process to run parametrization in parallel. Default is 1.')
#     add_pert_parser.add_argument('--submit', dest='submit', action='store_true', help='Toogle to submit the job')
#     add_pert_parser.add_argument('--run-gas', dest='skip_gas', action='store_false', help='Toogle to also perform gas-phase simulation. Useful when you want to analyze solvation contribution. Only used when submit is True.')
#     add_pert_parser.add_argument('--overwrite', dest='overwrite', action='store_true', help='Toggle to overwrite the exisiting perturbations.')
...
#     ana_parser.add_argument('-p', '--protein_name', dest='protein_name', default="", help='Name of the protein')
#     ana_parser.add_argument('-n', '--pert_name', dest='pert_name', default="", help='Name of the perturbation')
#     ana_parser.add_argument('--skip_traj', dest='skip_traj', action="store_true", help="Skip analyzing simulation trajectories")
#     ana_parser.add_argument('-m', '--num_workers', dest='num_workers', default=1, type=int, help="Number of processes to run the analysis in parallel")

#     # Report
#     report_parser = subparsers.add_parser("report", help='Report calculation results to a specified folder')
#     report_parser.add_argument('-o', '--output_dir', dest='save_dir', required=True, help='Output directory')
#     report_parser.add_argument('-p', '--protein', dest='protein', default='', help='Report data belongs to a specific protein model. If not specified, all data will be reported.')
#     report_parser.add_argument('-v', '--verbose', dest='verbose', action='store_true', help='Report verbose information for rbfe')
...
@click.group()
@click.option('--directory', '-d', type=str)
@click.option('--init', is_flag=True)
@click.pass_context
def main(ctx, directory, init):
    from .amber import AmberRbfeProject
    ctx.ensure_object(dict)
    if directory is not None:
        ctx.obj['project'] = AmberRbfeProject(directory, init=init)
...
@click.pass_context
def add_protein(ctx, **kwargs):
    project = ctx.obj.get('project')
    print(project, kwargs)