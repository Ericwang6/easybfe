...
from ..analysis.interaction import HBondFinder
...
class RxRxBoreschRestraintsFinder(BoreschRestraintsFinder):
    ...
    def find(self, protein: Protein, ligand: Ligand):
        ...
        candidates = []
        for record in hbond_data:
            pidx, lidx = record[0], record[1]
            ...
            for nei in ligand_atom.GetNeighbors():
                ...
                if ang1 < 45 or ang1 > 135 or ang2 < 45 or ang2 > 135:
                    continue
        ...
        restr = BoreschRestraint(protein_anchors=tuple(candidates[0][:3]), ligand_anchors=tuple(candidates[0][3:]))
